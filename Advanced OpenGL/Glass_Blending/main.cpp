...
int MARBLE_TEXTURE_ID=0,METAL_TEXTURE_ID=1,WINDOW_TEXTURE_ID=2;
int main() {
	glfwInit();
	...
	window_positions.push_back(glm::vec3(-1.5f, 0.0f, -0.48f));
	window_positions.push_back(glm::vec3( 1.5f, 0.0f, 0.51f));
	window_positions.push_back(glm::vec3( 0.0f, 0.0f, 0.7f));
	window_positions.push_back(glm::vec3(-0.3f, 0.0f, -2.3f));
	window_positions.push_back(glm::vec3( 0.5f, 0.0f, -0.6f));
	...
	std::string grass_path = std::string(ROOT_DIR)+ "textures/window.png";
	...
	Texture2d window_texture=Texture2d(grass_path.c_str(),WINDOW_TEXTURE_ID,true);
	marble.setWrapMode(GL_REPEAT,GL_REPEAT);
	metal.setWrapMode(GL_REPEAT,GL_REPEAT);
	window_texture.setWrapMode(GL_CLAMP_TO_EDGE,GL_CLAMP_TO_EDGE);
	...
	while (glfwWindowShouldClose(window) == 0) {
		...
		glBindVertexArray(grassVAO);
		shader.use();
		shader.setInt("texture1",WINDOW_TEXTURE_ID);
		window_texture.use();
		glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);

		// we need to render transparent objects in order, furthest to nearest, in order to prevent clipping
		//of transparent textures far away
		std::map<float,glm::vec3> sorted;


		for (int i=0;i<window_positions.size();i++) {
			//sorts the window from low to high distance
			float distance= glm::length(cameraPos-window_positions[i]);
