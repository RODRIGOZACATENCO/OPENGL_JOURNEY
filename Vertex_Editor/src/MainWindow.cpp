...
void MainWindow::use(){
	...
	if(update_faces_selected){
		for (auto mesh:meshes) {
			...
			glBindBuffer(GL_SHADER_STORAGE_BUFFER,ri->SSBO);
			glBufferData(GL_SHADER_STORAGE_BUFFER,m->face_is_selected.size()*sizeof(int),m->face_is_selected.data(),GL_DYNAMIC_DRAW);
			glBindBufferBase(GL_SHADER_STORAGE_BUFFER,0,ri->SSBO);
...
unsigned int MainWindow::mouseDetection(){
	...
	glReadPixels(framebuffer_x,framebuffer_y,1,1,GL_RED_INTEGER,GL_UNSIGNED_INT,&pixel_data);
...
void MainWindow::mainRenderPass() {
	...
	for (auto mesh:meshes) {
		...
		glDrawElements(GL_TRIANGLES,m->render_indices.size(),GL_UNSIGNED_INT,0);
		glBindVertexArray(0);
	}
}
...
void MainWindow::meshRenderSetup() {
	for (auto mesh:meshes) {
		...
		glBindBuffer(GL_SHADER_STORAGE_BUFFER,ri->SSBO);
		glBufferData(GL_SHADER_STORAGE_BUFFER,m->face_is_selected.size()*sizeof(int),m->face_is_selected.data(),GL_STATIC_DRAW);
		glBindBufferBase(GL_SHADER_STORAGE_BUFFER,0,ri->SSBO);
		glBindBuffer(GL_SHADER_STORAGE_BUFFER,0);
	...
	std::string geometry_shader_path="../shaders/shader.geom";
